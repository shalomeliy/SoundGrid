...
function ChannelStrip({ deckId }: { deckId: DeckId }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="flex flex-col items-center gap-1.5 rounded-[var(--radius-md)] bg-surface-0/60 p-1.5 shadow-[inset_0_0_0_1px_var(--color-hairline)]">
        <Knob label="Hi" size={38} value={ch.eqHigh} tone={color} onChange={(v) => ctl.setEq(deckId, 'high', v)} />
        <Knob label="Mid" size={38} value={ch.eqMid} tone={color} onChange={(v) => ctl.setEq(deckId, 'mid', v)} />
        <Knob label="Low" size={38} value={ch.eqLow} tone={color} onChange={(v) => ctl.setEq(deckId, 'low', v)} />
      </div>
      <Knob label="Filter" size={38} value={ch.filter} tone="var(--color-accent)" onChange={(v) => ctl.setFilter(deckId, v)} />
      <Fader
        label="Vol"
        value={ch.volume}
...
export function Mixer() {
  ...
  return (
    <section className="panel flex flex-col items-center gap-2.5 overflow-hidden p-3">
      ...
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-center gap-2.5 pt-6">
          ...
          <Knob label="Cue Vol" size={38} value={mixer.cueVolume} min={0} max={1} tone="var(--color-live)" onChange={ctl.setCueVolume} />
          <Knob label="Cue Mix" size={38} value={mixer.cueMix} min={0} max={1} tone="var(--color-live)" onChange={ctl.setCueMix} />
        </div>
