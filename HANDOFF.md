...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.6.0` (Sampler / Performance Decks) נסגרה 09/09. הקלטה מהמאסטר נדחתה בכוונה ל-v0.6.1 — ר' `ROADMAP.md`. **4 באגים נמצאו ותוקנו אחרי הסגירה** דרך שימוש אמיתי: גרירה-לסאמפלר (overlay תקוע), Mix Assist תקוע (`mixOnly`), ופרסיסטנס הסאמפלר על reload (שני באגים מצטברים — ר' פירוט). גם חובר כפתור ייצוא/ייבוא שהיה קיים בקוד אבל בלי כפתור בממשק. פרטים מלאים: [`docs/handoff/v0.6.0.md`](docs/handoff/v0.6.0.md). |
