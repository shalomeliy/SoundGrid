...
| **branch** | **עובדים ישירות על `main`** (החלטת המשתמש, 03/09). תיקון ברירת המחדל של Hint mode נדחף ישירות ל‑`main` כ‑fast-forward (04/09) — לוודא עם `git log --oneline origin/main -1` שהיא `1ed1150` או אחריה. ‏`claude/hints-missing-0-4-8-q48hk9` (שממנו הגיע התיקון) נשאר קיים ב‑remote, מיותר עכשיו. |
| **גרסה נוכחית** | `v0.4.8` |
