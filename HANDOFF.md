...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.6.0` (Sampler) נסגרה 09/09; הקלטה מהמאסטר נדחתה ל-v0.7.5 (ר' `ROADMAP.md`). **תוקנו אחרי הסגירה:** גרירה-לסאמפלר, Mix Assist תקוע, פרסיסטנס+מהירות על reload, אזהרה כוזבת, ייצוא/ייבוא בלי כפתור, rename לפד (Alt+click). פרטים: [`docs/handoff/v0.6.0.md`](docs/handoff/v0.6.0.md). |
...
| **הבא בתור** | **`v0.7.0` (FX, סיבוב ראשון) — spec מאושר** (09/10, סבב 4 מומחים): Delay/Echo/Reverb/Filter, ערוץ+מאסטר, FLX4 קבוע (לא MIDI Learn — כבר v0.11.0), פרסיסטנס, cue שומע FX מקדים. שאר 8 האפקטים המקוריים + Send → `v0.7.1`. פרטים: `workshop-output/FEATURE_SPEC.md`. **הבא בפועל: PLAN.md טכני (Plan Mode), עדיין לא נכתב.** |
