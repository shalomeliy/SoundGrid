...
| | |
| --- | --- |
| **בעבודה** | **`v0.7.0` (FX, סיבוב ראשון) — 11/12 צעדי `PLAN.md` בוצעו.** Delay/Echo/Reverb/Filter, ניתוב ערוץ+מאסטר, FX rack שקוף לחלוטין כש-off/לא מנותב (נבדק ב-RMS מול baseline), פרסיסטנס, FLX4 קבוע, זמן-ביט עוקב אחרי פדל הטמפו של דק המאסטר. נותר: צעד 12 (סבב רגרסיה מלא + `npm run check` + סגירת גרסה). פרטים: `workshop-output/FEATURE_SPEC.md` + `PLAN.md`. |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד** (הוראת שלום). `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.6.0` (`package.json`) — `v0.7.0` עדיין לא עודכן ב-`package.json`, ר' צעד 12. |
| **`npm run check`** | ירוק — מלבד 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | לסגור את v0.7.0 (צעד 12), אז `v0.7.1` (Bit Crusher/Roll/Send) או `v0.7.5` (הקלטה). |
