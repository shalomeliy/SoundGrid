...
- **`loadTrackToDeck` זורק שקט על כשל decode** (נמצא ב‑v0.4.0, שלב 8). קובץ שלא
  ניתן לפענוח (בניגוד לכשל *ניתוח* — decode עצמו נכשל) גורם ל‑unhandled rejection;
  אף call site (double‑click ב‑`Library.tsx`, drop ב‑`Deck.tsx`) לא תופס ומציג
  הודעה. בדיוק ה"דילוג השקט" ש‑CLAUDE.md אוסר. הוצע כ‑task נפרד (רשימת ה‑tasks של
  הסשן); פרטים: [`docs/handoff/v0.4.0.md`](docs/handoff/v0.4.0.md).
