...
| | |
| --- | --- |
| **בעבודה** | אין. ‏`v0.2.6` · `v0.2.7` · `v0.2.8` · `v0.2.9` · `v0.2.10` נסגרו. מה מהן על main — `git fetch && git log --oneline origin/main -5`, לא השורה הזאת |
...
| **גרסה נוכחית** | `v0.2.10` — וזה גם מה ש‑`package.json` אומר. בדיקה מקבעת את השוויון (`tests/repo/version-in-step.test.ts`); **לשנות את שניהם יחד** |
...
**כלום בקוד.** ‏v0.2.0 · v0.1.7 · v0.2.5 · v0.2.6 · v0.2.7 · v0.2.8 · v0.2.9 · v0.2.10
סגורות, והקריסה הלא‑מאומתת מ‑28/08 נסגרה (לא השתחזרה). הרשומה של הסבב הזה, כולל
המספרים שנמדדו: [`docs/handoff/v0.2.6.md`](docs/handoff/v0.2.6.md) ·
[`v0.2.8.md`](docs/handoff/v0.2.8.md).

**v0.2.10 — עמודת ז'אנר**, נגזרת מתיקיית האב בסריקה (`core/genres.ts`, התאמה
מנורמלת + כינויים). עריכה ידנית מרשימה סגורה, נשמרת ב‑IndexedDB ייעודי ושורדת
סריקה חוזרת. פירוט: `workshop-output/FEATURE_SPEC.md` ו‑`PLAN.md`.
...
- **עריכת ז'אנר ידנית אובדת בשקט אם הקובץ זז לתיקייה אחרת** (v0.2.10). מזהה הטראק
  בנוי מהנתיב היחסי; העברת קובץ לתיקיית ז'אנר אחרת ואז סריקה חוזרת משנה את המזהה,
  וההתאמה לעריכה הישנה לא נמצאת יותר — חוב שהמשתמש קיבל והסכים לו במפורש, לא תוקן.
