...
| | |
| --- | --- |
| **בעבודה** | `v0.5.5` שלב 1 סגור. שלב 2 (מודל מקומי, `platform/ai-local/`, CPU-only מ-09/09 אחרי ש-GPU תקע את השבב האמיתי) **עבד בפועל על המחשב של שלום: "נגן דק A" → הבין, אישר, ניגן — ~דקה.** ניסיון שני ("סנכרון דק B") נתקע 120s ונפל יפה ל‑watchdog (הודעה גלויה, לא תקיעה שקטה — בדיוק כמו שתוכנן). **חשד לסיבת האיטיות/חוסר-העקביות: Qwen3 "חושב בקול" (`<think>`) לפני שעונה** — תוקן: `tokenizer_encode_kwargs:{enable_thinking:false}` ב‑`worker.ts`. **עוד לא נבדק.** גם נוסף: אזהרת "פעם ראשונה" (`first-run-warning`) לפני ההורדה הראשונה — "זה רץ על המחשב שלך, יכול לקחת דקה-שתיים, לפעמים לא לענות" + כפתור אישור, נשמר ב‑IndexedDB כך שמוצג פעם אחת בלבד (`warningAck.ts`). זה במקום שער "יש לך מחשב חזק?" ששלום הציע — אין דרך אמיתית למשתמש לדעת לענות על זה. |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד, בכל שיחה** (הוראת שלום מפורשת). לבדוק: `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.5.4` (`package.json`) — שלב 2 עדיין לא סגור (תיקון ה‑thinking לא נבדק על אמת). |
| **`npm run check`** | ירוק — מלבד אותם 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | שלום עושה `git pull`, מרענן, ומנסה שוב מהתחלה (יראה את האזהרה החדשה פעם אחת, ילחץ "Try it"). שאלה יחידה שחשובה: **האם התשובה מגיעה מהר יותר ויותר עקבי** אחרי ביטול ה‑thinking? אם כן — ממשיכים למדידת קורפוס מלא (סגנון v0.1.7) וסוגרים את הגרסה. אם עדיין לא-עקבי/איטי — זו מגבלת CPU-only גרידא, לא באג, ואז השאלה חוזרת לשלום: לחיות עם זה, או לשקול BYO-key. |
