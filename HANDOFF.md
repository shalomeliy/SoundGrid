...
| **הבא בתור** | `v0.8.0` — **ניהול ספרייה** (crates, עמודות, היסטוריה). קודמה 09/11 לפני `v0.7.1`. |
...
- **סדר הבנייה (09/11):** `v0.8.0` (ניהול ספרייה) קודמה לפני `v0.7.1`
  (FX Bit Crusher/Roll) — בקשת שלום. `v0.7.1` לא בוטלה, רק נדחתה.
