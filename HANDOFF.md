...
- **הבא בתור:** ① לסגור את שלושת הממצאים הפתוחים (סעיף ⚠️ למטה — A הוא באג אמיתי,
  B ו‑C הערות שקריות) ② להכשיל את ה‑worklet בכוונה ולוודא שהפיל `no scratch` מופיע —
  **רק אחרי A**, אחרת האימות ייכשל מסיבה לא נכונה ③ לבדוק את v0.2.0 עם ה‑FLX4 ועם
  העכבר ④ ואז v0.3.0 (beatgrid)
...
### ⚠️ שלושה ממצאים שעדיין פתוחים (אומתו בקוד 2026-08-29)

רשמתי קודם ששניים מאלה נסגרו ב‑`8ead1d1`. **טעיתי** — בדקתי שהחוליות קיימות ולא עקבתי
עד הסוף. סבב אימות נוסף מצא:
...
**A הוא היחיד שהוא באג אמיתי; B ו‑C הם תיעוד שקרי.** את A צריך לתקן לפני שמנסים לאמת
את הפיל, אחרת האימות ייכשל מסיבה לא נכונה.
