...
| | |
| --- | --- |
| **בעבודה** | `v0.8.5` M1 בנוי. **M2 נמדד על מחשב שלום (15/09):** טעינה 8.1
  שנ' (חד-פעמי), אז ~0.3 שנ'/טראק **בלי קשר לאורך** (30 שנ' ⇐ 0.32, 3.5 דק'
  ⇐ 0.26) — כי `ClapFeatureExtractor` חותך כל טראק ארוך לקטע **אקראי** קבוע
  לפני שהוא נשמע, לא מנתח את כל השיר. לא רק הבדל מהירות מהקלאסי (M1, כל
  הקובץ) — סיכון איכות. חסם לפני החלטה: לשאול את שלום עם שני המספרים +
  ההשפעה על איכות, לא להניח ש"AI חכם" עדיין תקף. ר' `PLAN.md` §שער M2. |
