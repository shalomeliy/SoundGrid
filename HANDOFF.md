...
| | |
| --- | --- |
| **בעבודה** | **שום דבר גדול כרגע.** v0.4.8 נסגרה (04/09) — ר' [`docs/handoff/v0.4.8.md`](docs/handoff/v0.4.8.md). תיקון קטן עוקב (04/09): ברירת המחדל של Hint mode הודלקה — ר' "החלטות שנסגרו" למטה. |
| **branch** | ברירת המחדל היא לעבוד ישירות על `main` (החלטת המשתמש, 03/09). **חריגה זמנית:** תיקון ברירת המחדל של Hint mode יושב ונדחף לענף `claude/hints-missing-0-4-8-q48hk9` (commit `ca3600e`) — לבדוק עם `git merge-base --is-ancestor ca3600e origin/main` אם כבר מוזג (exit 0 = כן). למזג או לסגור לפני שפותחים גרסה חדשה. |
| **גרסה נוכחית** | `v0.4.8` |
| **`npm run check`** | ירוק (04/09, אחרי `npm install` מלא). **בלי לצטט כאן מספר בדיקות** — ר' `tests/repo/handoff-counts.test.ts` |
...
**מה פתוח בקוד:** שום גרסה באמצע כרגע; ענף לא-ממוזג אחד (ר' שורת branch למעלה).
החובות הטכניים וההחלטות הממתינות למטה.
