...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.6.0` (Sampler / Performance Decks) נסגרה 09/09. הקלטה מהמאסטר נדחתה בכוונה ל-v0.6.1 — ר' `ROADMAP.md`. **תוקן אחרי הסגירה:** שני סבבי גרירה-לסאמפלר (overlay שנתקע דלוק — פרטים ב-[`docs/handoff/v0.6.0.md`](docs/handoff/v0.6.0.md)), ועכשיו עוד באג נפרד: שלום שלח צילום מסך — תיקייה עם 43 שירים הציגה "No audio files found". סיבה: Mix Assist (`mixOnly`) נדלק לבד כשדק מתחיל לנגן ולא כיבה את עצמו כשהניגון נעצר, והכפתור שהיה מכבה אותו נעלם יחד איתו — הרשימה נעלמה מאחורי פילטר בלתי נראה. תוקן: האפקט מכבה `mixOnly` כשגם דק A וגם B נעצרים; והודעת ה-empty-state (`core/library-list-copy.ts`, בדיקות ב-`tests/core/`) מזהה עכשיו את הסיבה האמיתית במקום לטעון שהתיקייה ריקה, עם כפתור "Show all tracks". |
...
| **הבא בתור** | `v0.6.1` (הקלטת סאמפלר מהמאסטר) או `v0.7.0` (FX Units) — לשלום להחליט. ר' `ROADMAP.md`. עדיין לא נכתב spec. |
...
- **הסאמפלר (v0.6.0) לא נבנה עם הקלטה מהמאסטר** — רק טעינה בגרירה מהספרייה.
  הקלטה דורשת הרחבת מנוע (הקשבה ל-bus, דיקוד חי) — v0.6.1 מועמד.
