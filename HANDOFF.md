...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.7.0` (FX, סיבוב ראשון) נסגרה 09/10 — Delay/Echo/Reverb/Filter, ניתוב ערוץ+מאסטר, פרסיסטנס, FLX4 קבוע. שני באגים אמיתיים נמצאו ותוקנו (לא רק תוכננו): `setEffect` קרס בקריאה ראשונה, ו-store תיעד "Reverb נבחר" גם כשהמנוע סירב. גם: רצועת UI ראשונה נמדדה ב-1536×710 ודחקה את הדקים לחיתוך שקט — תוקנה. פרטים מלאים: [`docs/handoff/v0.7.0.md`](docs/handoff/v0.7.0.md). |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד** (הוראת שלום). `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.7.0` (`package.json`) — נסגרה 09/10. |
| **`npm run check`** | ירוק — מלבד 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | `v0.7.1` (Bit Crusher/Roll/Send, AudioWorklet) או `v0.7.5` (הקלטה מאוחדת) — לשלום להחליט. |
...
- **הסאמפלר (v0.6.0) לא נבנה עם הקלטה מהמאסטר** — רק טעינה בגרירה מהספרייה.
  הקלטה דורשת הרחבת מנוע (הקשבה ל-bus, דיקוד חי) — v0.7.5 מועמד.
...
- **אין בינדינג FLX4 לעריכת סלוט סאמפלר** (מצב/גיין/sync/מחיקה) — עכבר
  בלבד, אותה מגבלה כמו ארבעת בינדינגי v0.5.0.
- **10 בינדינגי FX חדשים ב-`flx4.ts` (v0.7.0) לא מאומתים על חומרה אמיתית** —
  אותה מגבלה כמו v0.5.0: אין FLX4 בסביבה המרוחקת. לתקן דרך Learn אם לחיצה/סיבוב
  לא עושה כלום.
- **זמן FX מסונכרן-ביט עוקב רק אחרי פדל הטמפו של הדק שכבר master** — לא אחרי
  SYNC שנדלק או אחרי שהמאסטר משתנה (`setMasterDeck`/`syncDeck`, לא נגעו בגרסה
  הזו כדי לא להסתכן ברגרסיה בקוד SYNC/מעברים). אם FX נשמע "תקוע" על BPM ישן
  אחרי שינוי master — לגעת בפדל הטמפו פעם אחת מתקן.
