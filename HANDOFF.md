...
| | |
| --- | --- |
| **בעבודה** | `v0.8.8` (יישור פעימה חי, SYNC) — קוד ממוזג, 12/12 Playwright,
  **ממתין לבדיקת שלום במיקס אמיתי**. `v0.8.5` M3 גם ממוזג, שלום אישר
  שעובד — סגירה רשמית ממתינה לאישור איכות. GridFix נבנה חלקית והוסר (16/09). |
| **⚠ נמצא 17/09** | `v0.8.6` סומן ✅ אבל קומיט אחד (גובה פאנל Transition
  Points) לא הגיע ל‑main — נשאר ב-branch יתום, תפס את שלום חי (כפתור Play
  מוסתר). שוחזר (PR #27). **לקח: "✅" ב‑ROADMAP לא מוכיח שהקוד ב‑main.** |
| **branch** | `main`, הכל ממוזג (`git log --oneline -1`). |
| **גרסה נוכחית** | `v0.8.7` (`package.json`) — נסגרה; `v0.8.8` בעבודה. |
| **`check`** | ירוק — 527/527. |
| **הבא בתור** | שלום בודק "◂ beat / beat ▸" במיקס אמיתי → v0.8.8 נסגר, אחר-כך `v0.8.9`. |
