...
| **branch** | ‏`claude/tracks-genre-column-frk51b` — `v0.3.2` (עמודת ז'אנר), נבנתה על main ישן (לפני שv0.3.0 נסגרה) ואוחדה עם main מחדש. לפני כל עבודה חדשה: `git fetch --prune && git branch -r --no-merged origin/main` — ענף שלא מוזג הוא claim על גרסה, לא לדרוך עליו |
...
- **עריכת ז'אנר ידנית אובדת בשקט אם הקובץ זז לתיקייה אחרת** (v0.3.2). מזהה הטראק
