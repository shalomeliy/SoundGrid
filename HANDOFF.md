...
| | |
| --- | --- |
| **בעבודה** | `v0.5.5` שלב 1 סגור. שלב 2 (מודל מקומי, CPU-only מ-09/09 אחרי ש-GPU תקע שבב אמיתי): שרשרת תיקונים על סמך בדיקות אמיתיות של שלום — (1) `enable_thinking:false` (ביטול "חשיבה בקול" של Qwen3), (2) כותרות `crossOriginIsolated` ב-`vite.config.ts` כדי שה-WASM ירוץ על כמה threads (**לא** "צריך כרטיס גרפי" כמו שלום חשב — הכרטיס מכובה בכוונה). **תוצאה מעורבת:** "תנגן את דק A" (ניסוח חופשי, לא מהקורפוס!) הובן נכון והוצע לאישור — סימן טוב — אבל **~1:52 של "חושב" ואז רק 8s לאשר, וזה בדיוק מה שפג.** לא ידוע אם התיקון של ה-threads בכלל היה פעיל בבדיקה הזאת (**דורש הפעלה מחדש של `npm run dev`, לא רק רפרוש** — לא ודאי ששלום עשה את זה). תוקן בנפרד, בלי תלות בזה: `AI_PROPOSAL_EXPIRY_MS` 8000→20000 (`controls.ts`) — היה מכוון למודל המזויף המיידי. `credentialless` עדיין לא מאומת מול huggingface.co האמיתי. |
...
| **הבא בתור** | שלום עושה `git pull`, מפעיל מחדש `npm run dev` (שינוי ב-`vite.config.ts` דורש הפעלה מחדש), מרענן, מנסה "תנגן את דק A" שוב — **הפעם עם 20 שניות לאשר.** אם עובד סוף לסוף מקצה־לקצה: ממשיכים למדידת קורפוס קצרה (כמה פקודות שונות, כולל דחייה/הבהרה) וסוגרים את הגרסה. אם ההורדה עצמה נשברת (שגיאת רשת חדשה) — `credentialless` מתנגש עם huggingface.co, מורידים את שתי הכותרות מ-`vite.config.ts`. |
