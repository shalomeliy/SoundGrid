...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.6.0` (Sampler / Performance Decks) נסגרה 09/09. הקלטה מהמאסטר נדחתה בכוונה ל-v0.7.5 (מוזזת קדימה מ-v0.10.0, מאוחדת עם הקלטת סאמפלר — בקשת שלום 09/10) — ר' `ROADMAP.md`. **תוקנו אחרי הסגירה, דרך שימוש אמיתי:** גרירה-לסאמפלר (overlay תקוע), Mix Assist תקוע (`mixOnly`), ופרסיסטנס+מהירות הסאמפלר על reload (3 באגים מצטברים, ועוד רגרסיה אחת
שסקירה עצמאית תפסה לפני שלום — הודעת אזהרה כוזבת — ר' פירוט). גם: כפתור ייצוא/ייבוא שהיה קיים בקוד בלי כפתור, ו-rename לפד סאמפלר (Alt+click, לא היה קיים בכלל — סקירה שנייה תפסה עוד קצה שבור בגרסה הראשונה שלו, גם תוקן). פרטים מלאים: [`docs/handoff/v0.6.0.md`](docs/handoff/v0.6.0.md). |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד, בכל שיחה** (הוראת שלום מפורשת). לבדוק: `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.6.0` (`package.json`) — נסגרה 09/09. |
| **`npm run check`** | ירוק — מלבד אותם 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | סדר נקבע (שלום, 09/10): **`v0.7.0` (FX Units) קודם, אחר כך `v0.7.5`** (הקלטה מאוחדת — מאסטר + סלוט סאמפלר, מוזזת מ-v0.10.0/v0.6.1 כדי לתפוס את המאסטר אחרי שרשרת ה-FX). ר' `ROADMAP.md`. מתחילים עכשיו ב-v0.7.0 — עדיין לא נכתב spec. |
