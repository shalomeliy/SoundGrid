...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.5.5` (שליטה בשפה טבעית) נסגרה 09/09 — **מכובה בכוונה, לא נכשלה טכנית.** שלב 2 (מודל מקומי) עבד נכון על המחשב האמיתי של שלום אחרי שרשרת תיקונים (GPU→CPU אחרי `DXGI_ERROR_DEVICE_HUNG`, ביטול "חשיבה בקול" של Qwen3, `crossOriginIsolated` ל-threading) — אבל נשאר ~1-2 דקות לפקודה, לא מתאים לדי-ג'י חי. **שלום החליט לעצור כאן** ולא לדחוף עוד תיקוני-מהירות — זו מגבלת מעבד-בלי-GPU-בטוח, לא באג. פרטים מלאים: [`docs/handoff/v0.5.5.md`](docs/handoff/v0.5.5.md). |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד, בכל שיחה** (הוראת שלום מפורשת). לבדוק: `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.5.5` (`package.json`) — נסגרה 09/09. |
| **`npm run check`** | ירוק — מלבד אותם 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | `v0.6.0` — Sampler / Performance Decks. ר' `ROADMAP.md`. עדיין לא נכתב spec. |
