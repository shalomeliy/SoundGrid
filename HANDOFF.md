...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.6.0` (Sampler / Performance Decks) נסגרה 09/09 — בנק גלובלי של 16 סלוטים, טעינה בגרירה, 3 מצבי נגינה, sync ל-BPM המאסטר, ערוץ מיקסר נפרד, פרסיסטנס+ייצוא/ייבוא. **הקלטה מהמאסטר לא נבנתה** (נדחתה ל-v0.6.1 בכוונה — ר' פרטים). **תוקן אחרי הסגירה, שני סבבים:** שלום מצא שגרירה לפדים לא עבדה (drop-zone של הדק עטף גם את הגריד), ואז מצא שהתיקון הראשון (`stopPropagation`) תקע את ה-overlay דלוק לצמיתות. התיקון הסופי: `data-drop-zone` + `Deck.tsx` בודק ומוותר בעצמו, בלי לחסום אירועים — אומת בסימולציית גרירה מלאה. פרטים מלאים: [`docs/handoff/v0.6.0.md`](docs/handoff/v0.6.0.md). |
