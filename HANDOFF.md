...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.7.5` (הקלטה מאוחדת — סאמפלר+מאסטר) נסגרה 09/11 — הקלטת סלוט חיה (שורדת רענון), הקלטת מאסטר ל-WAV עם תקרת זיכרון וחיתוך לפי טראק. כמה באגים נמצאו ותוקנו — בבדיקה חיה (פרסיסטנס מתועכב, מונה זמן "-1:-1") **וגם** ע"י `change-reviewer` (תקרת זיכרון לא נאכפת לסלוט סאמפלר, מחיקת blob שקטה, מירוץ Save/Discard). פרטים: [`docs/handoff/v0.7.5.md`](docs/handoff/v0.7.5.md). |
...
| **הבא בתור** | `v0.7.1` (Bit Crusher/Roll/Send FX) או `v0.8.0` (ניהול ספרייה) — לשלום להחליט. |
