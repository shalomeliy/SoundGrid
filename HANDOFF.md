...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.6.0` (Sampler / Performance Decks) נסגרה 09/09. הקלטה מהמאסטר נדחתה בכוונה ל-v0.6.1 — ר' `ROADMAP.md`. **תוקנו אחרי הסגירה, דרך שימוש אמיתי:** גרירה-לסאמפלר (overlay תקוע), Mix Assist תקוע (`mixOnly`), ופרסיסטנס+מהירות הסאמפלר על reload (3 באגים מצטברים — ר' פירוט). גם: כפתור ייצוא/ייבוא שהיה קיים בקוד בלי כפתור, ו-rename לפד סאמפלר (Alt+click, לא היה קיים בכלל). פרטים מלאים: [`docs/handoff/v0.6.0.md`](docs/handoff/v0.6.0.md). |
