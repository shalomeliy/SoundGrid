...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.7.5` נסגרה 09/11 — פרטים: [`docs/handoff/v0.7.5.md`](docs/handoff/v0.7.5.md). |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד** (הוראת שלום). `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.7.5` (`package.json`) — נסגרה 09/11. |
| **`npm run check`** | ירוק — מלבד 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | `v0.8.0` — **עמודות ממיינות + חיפוש** (שלב 1 מ‑4 של ניהול ספרייה, פוצלה 09/11). איפיון: `workshop-output/FEATURE_SPEC.md`. |
