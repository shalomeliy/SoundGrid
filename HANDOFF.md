...
- **הבא בתור:** ① **פיצול המסמכים + אינווריאנטים ברמת הריפו** — ראה "חוב תשתית התיעוד"
  ...
  ② להכשיל את ה‑worklet בכוונה ולוודא שהפיל `no scratch` מופיע (הפריט האחרון שנשאר
  פתוח ב‑definition of done של v0.2.0) ③ לבדוק את v0.2.0 עם ה‑FLX4 ועם העכבר
  ④ ואז v0.3.0 (beatgrid)
...
**הפריט שנשאר פתוח מהסקירה הוא לא ממצא אלא אימות:** אף אחד עדיין לא הכשיל את ה‑worklet
בכוונה כדי לראות שהפיל `no scratch · <reason>` באמת מופיע. עכשיו שהשרשרת שלמה, זה סוף‑סוף
אימות שיכול לעבור.
