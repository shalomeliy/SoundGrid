...
| | |
| --- | --- |
| **בעבודה** | `v0.8.6` — סגירת חובות טכניים פתוחים (בקשת המשתמש 13/09) + באג crate שהתגלה תוך כדי ("9 not found" על ספרייה תקינה). |
| **branch** | `claude/quirky-ptolemy-2qeikv`, מכיל את `main` (`git merge-base --is-ancestor 79d877a origin/main`). |
| **גרסה נוכחית** | `v0.8.6` (`package.json`) — בעבודה. |
| **`npm run check`** | ירוק — מלבד 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור. |
| **הבא בתור** | סיום `v0.8.6`, אחר כך `v0.8.4` — פרטיות/תנאי שימוש כנים + תיקון נגישות אמיתי, `ROADMAP.md`. |
