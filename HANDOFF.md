...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה כרגע.** `v0.5.1` (תיקון נקודות המעבר) נסגרה (09/09) — ביקוש ישיר של שלום אחרי שימוש בפועל: "into the outro" הוסר לגמרי (תמיד נמצא, אף פעם לא שימושי), זיהוי אמצע-שיר שופץ לסף יחסי-מקומי (תופס גם ברייקדאון בשיר ממוסטר בדחיסות גבוהה) ומזהה גם עליות אנרגיה (`energy-lifts`), לא רק ירידות. פרטים מלאים: [`docs/handoff/v0.5.1.md`](docs/handoff/v0.5.1.md). |
...
| **`npm run check`** | ירוק (09/09, על `v0.5.1`, כולל סגירתה) — מלבד 8 כשלים pre-existing ב‑`doc-commits.test.ts` שמקורם בשיבוט git רדוד בקונטיינר המרוחק (SHA-ים ישנים מהתיעוד לא נגישים), לא קשור לדיף. **בלי לצטט כאן מספר בדיקות** — ר' `tests/repo/handoff-counts.test.ts` |
| **הבא בתור** | `v0.5.5` — שליטה בשפה טבעית/קול. ר' `ROADMAP.md`. עדיין לא נכתב spec. **אבל ר' גם** את הבקשה הפתוחה מתחת על נקודת-יציאה לדק היוצא — שלום ביקש אותה באותה שיחה, ייתכן שהיא קודמת ל‑v0.5.5 בפועל. |
...
- **נקודת יציאה לדק היוצא (09/09, לא ספק v0.5.1)** — שלום ביקש: לזהות גם מתי הדק
  שכבר מנגן צריך לרדת מההשמעה ולהשאיר רק את הנכנס, לא רק איפה להיכנס. כרגע
  `startAutoTransition` (`controls.ts`) משתמש ב‑`TRANSITION_CROSSFADE_SEC` קבוע (8ש',
  `core/constants.ts`) — לא מודע למבנה של אף דק. הרעיון הטבעי: להשתמש באותם מועמדים
  שכבר מחושבים (`findTransitionCandidates`) גם על הדק היוצא, כ"נקודת יציאה" מוצעת —
  אבל צריך להחליט: משך קרוספייד דינמי במקום קבוע? אינדיקציה ויזואלית חדשה על הדק
  היוצא? מה קורה כשאין מועמד יציאה מתאים (רחוק מדי / קרוב מדי לנוכחי)? פרטים:
  [`docs/handoff/v0.5.1.md`](docs/handoff/v0.5.1.md) סעיף "חובות/פתוח".
