...
| | |
| --- | --- |
| **בעבודה** | **שום גרסה כרגע.** `v0.4.6` (Mix Assist) נסגרה 05/09 — התכולה המלאה ומה שנמדד ב‑[`docs/handoff/v0.4.6.md`](docs/handoff/v0.4.6.md). `v0.4.7` (קרבת אנרגיה למיקס) הבא בתור, **בלי איפיון עדיין** — סבב מוצר/ארכיטקטורה/עיצוב/QA משלה לפני קוד. |
| **branch** | **עובדים ישירות על `main`** (החלטת המשתמש, 03/09), דרך branch+PR שממוזג fast-forward. `claude/handoff-documentation-a8inr7` **עדיין לא ממוזג ל‑`main`** — לוודא עם `git log --oneline origin/claude/handoff-documentation-a8inr7 -1`. |
| **גרסה נוכחית** | `v0.4.8` (`package.json`). |
| **`npm run check`** | ירוק (05/09). **בלי לצטט כאן מספר בדיקות** — ר' `tests/repo/handoff-counts.test.ts` |
| **הבא בתור** | לפתוח סבב איפיון ל‑`v0.4.7` (מוצר/ארכיטקטורה/עיצוב/QA) לפני קוד — ר' `ROADMAP.md`. |
