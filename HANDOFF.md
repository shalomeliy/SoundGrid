...
| | |
| --- | --- |
| **בעבודה** | `v0.5.5` שלב 1 סגור. שלב 2 (מודל מקומי אמיתי, `platform/ai-local/`) **אומת על המחשב האמיתי של שלום (09/09): המודל יורד ועולה בהצלחה.** אבל הריצה הראשונה (`chat()`, `device:'auto'`) **תקעה את שבב הגרפי שלו בפועל** — `DXGI_ERROR_DEVICE_HUNG` בקונסולה, מעבד רגוע (לא "לאט" — תקוע ממש, אין דרך לתפוס את זה אחרי המעשה כי שבב שקרס לא דוחה promise ולא קורא ל‑`onerror`). **תוקן:** `device:'wasm'`+`dtype:'q8'` קבועים (בלי GPU כלל — מיפוי ברירת המחדל של הספרייה עצמה, לא ניחוש), וטיימר‑שומר (watchdog) של 120s בכל בקשה ל‑worker שמפיל שקט מוחלט להודעת שגיאה גלויה. **עוד לא נבדק אחרי התיקון.** בדרך גם תוקנה תקלת build לא-קשורה: `new Worker(new URL(...))` (כאן וב‑`analyzer-worker`) לא מתאגד נכון ב‑Vite 8 — `?worker` import הוא הפאץ', `tests/repo/worker-import-syntax.test.ts` חוסם חזרה. |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד, בכל שיחה** (הוראת שלום מפורשת — לא לפתוח branches). לבדוק: `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.5.4` (`package.json`) — שלב 2 עדיין לא סגור (התיקון האחרון לא נבדק). |
| **`npm run check`** | ירוק — מלבד אותם 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | שלום מרפרש את הדף (ה‑WebGPU context קרס) ומנסה שוב: מדליק AI, מקליד פקודה. שלוש תוצאות: (1) תשובה תוך דקה‑שתיים, נכונה — ממשיכים למדידת קורפוס (סגנון v0.1.7); (2) הודעת שגיאה אחרת — שולח לי בדיוק מה; (3) 120s בלי תגובה — עכשיו תופיע הודעה במקום תקיעה שקטה, שולח לי גם את זה. |
