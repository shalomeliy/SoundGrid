...
| | |
| --- | --- |
| **בעבודה** | **v0.2.6 — פתיחה עם הספרייה כבר טעונה.** האיפיון ב‑[`ROADMAP.md`](ROADMAP.md). ‏`v0.2.5` ו‑`v0.2.0` סגורות ונדחפו |
...
| **הבא בתור** | v0.3.0 — Beatgrid & phase‑sync |
...
‏`public/midi-check.html` הוא מוניטור ה‑MIDI שמצא שם את באג ה 62 — קוראים את
הסכום לדק מעל הטבלה, לא שורה בודדת.

**פתוח מ‑v0.1.7 — היחיד שנשאר:** לאמת **בעין** שעמודות Artist/BPM/Key/Time
בספרייה מתמלאות ושפריסת 7 העמודות סבירה. הפרסר נבדק מול 360 קבצים; הטבלה
עצמה לא נבדקה על מסך. **המשתמש יצלם** (02/09).
