...
| | |
| --- | --- |
| **בעבודה** | **אין גרסה פתוחה.** `v0.8.0` נסגרה 09/11 — פרטים: [`docs/handoff/v0.8.0.md`](docs/handoff/v0.8.0.md). |
| **branch** | הוראת שלום מ‑09/09: `main` ישירות (`git status` מראה זאת). שיחה מרוחקת זו קיבלה branch נפרד (`claude/wonderful-cori-scf85f`) מהגדרת הסביבה — אילוץ, לא בחירה. |
| **גרסה נוכחית** | `v0.8.0` (`package.json`) — נסגרה 09/11. |
| **`npm run check`** | ירוק — מלבד 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | `v0.8.1` — Crates (עץ, גרירה, crate חכם עם רענון ידני). ר' `ROADMAP.md`. |
...
- **שני סקריפטי אימות ישנים התיישנו** מול גרסאות מאוחרות מהן עצמן
  (`verify-pad-modes.mjs`, `verify-settings.mjs`/`verify-library-boot.mjs`)
  — לא קשור לדיף v0.7.5 (אומת ב‑`git log`), לא תוקן כדי לא להרחיב היקף.

---

## החלטות שנסגרו

- **סדר הבנייה (09/11):** ניהול ספרייה קודמה לפני `v0.7.1` (FX Bit
  Crusher/Roll) — בקשת שלום. `v0.7.1` לא בוטלה, רק נדחתה.
- **היקף ניהול הספרייה (09/11):** פוצל ל‑4 גרסאות (`v0.8.0` עמודות+חיפוש
  → `v0.8.1` crates → `v0.8.2` "מתנגן עכשיו" → `v0.8.3` היסטוריה מלאה,
  נמוך-עדיפות) במקום גרסה אחת — בחירת שלום. דירוג (⭐) הוסר. `ROADMAP.md`.
