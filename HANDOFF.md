...
**מה פתוח בקוד:** `v0.4.6` באמצע (שלבים 1‑6/10, ר' למעלה). כל שינוי חדש בקוד ממשיך אותה, לא פותח גרסה מקבילה.

**שלב 6 אומת בדפדפן אמיתי (Chromium headless, 04/09)** — לא ספריית המשתמש (אין אותה בקונטיינר מרוחק): שני קבצי WAV סינתטיים (אחד עולה באנרגיה ב‑5 שניות, אחד שטוח) הוכיחו שהפאנל מופיע/נעלם נכון, מציג את הודעת "אין נקודות" הכנה על הטראק השטוח, ואת "0:05 · past the intro — energy builds" על השני; קליק עליה הזיז את מיקום הדק. אין שגיאות קונסולה.
