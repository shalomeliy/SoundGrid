...
| **הבא בתור** | `v0.5.5` — שליטה בשפה טבעית (הקלדה בלבד, לא קול — הוחלט 09/09). Spec מאושר: [`workshop-output/FEATURE_SPEC.md`](workshop-output/FEATURE_SPEC.md). תכנון טכני מוכן: [`workshop-output/PLAN.md`](workshop-output/PLAN.md) — שני שלבים (1: הצנרת המלאה מול AI מדומה, נבדקת כאן; 2: המודל המקומי האמיתי, spike נפרד — ספריית WebGPU/WASM עוד לא נבחרה). **ממתין לאישור שלום על התכנית לפני מימוש.** |
