...
| | |
| --- | --- |
| **בעבודה** | **`v0.5.5` שלב 1 מומש ונבדק (09/09) — שלב 2 עדיין לא התחיל, הגרסה עוד לא סגורה.** הצנרת המלאה: `core/ports/ai.ts` הורחב (`chat()`/tool-calling לצד ה-`suggest()` הישן), `core/ai/toolCatalog.ts` חדש (קטלוג כלים + שער ולידציה, טהור), `platform/ai-mock/` (provider מדומה ודטרמיניסטי לבדיקות), `controls.ts` קיבל `submitAiCommand`/`confirmAiProposal`/`cancelAiProposal`/`toggleAiControl` (state machine idle→thinking→confirm/clarify/decline, תפוגה אוטומטית עם הודעה), `AiControlBar.tsx` חדש + כפתור ב-`TopBar`. בדיקות vitest חדשות + בדיקות Playwright בדפדפן (כל קריטריוני הקבלה מה-spec מלבד איכות מודל אמיתי — זה שלב 2) — `npm run check` ירוק. עדיין לא עבר `change-reviewer`. |
