...
- **הבא בתור:** ① לסגור את ממצאי סקירת v0.2.0a (הסעיף המסומן ⚠️ מיד למטה — שני חוסמים
  פתוחים) ② לבדוק את v0.2.0 עם ה‑FLX4 ועם העכבר ③ ואז v0.3.0 (beatgrid)
...
### ⚠️ סקירת v0.2.0a חזרה REPAIR — פתוח (2026-08-29)

השיחה שכתבה את v0.2.0a (המנוע, `fe6ff9e` + `dd7a3ba`) קיבלה סקירה עצמאית והעבירה את
הממצאים במקום לתקן, כדי לא לדרוס את השיחה שכתבה את v0.2.0b באותו עץ עבודה.
**את ארבעת הסעיפים בטבלה אימתתי בקוד**; האזהרות שאחריה מועברות כלשונן ולא נבדקו.

**עץ העבודה נקי.** התיקון שתואר בהעברה כ"יושב לא‑מקומט" כבר נכנס ב‑commits של v0.2.0b.

| # | הממצא | מצב | היכן |
| --- | --- | --- | --- |
| 1 | **חוסם — `ended` מתנגש ב‑`seek` ומשאיר דק מת בשקט.** סינון ה‑epoch חל גם על `ended`. אם טראק מגיע לקצה וקורה seek לפני שההודעה נקלטה, היא נזרקת, `onEnd` לא רץ, `_playing` נשאר true, ו‑`process()` חוזר ריק לנצח — שקט בזמן שהפלייהד מתקדם, ו‑`play()` לא עושה כלום. המשתמש חייב פאוזה ואז פליי. קפיצה לקראת סוף טראק היא התנהגות DJ רגילה. **תיקון:** לפטור `ended` מסינון ה‑epoch (זו טרנזיציה סופית, לא דגימת מיקום) ולתלות אותו ב‑`this.playing`, כדי ש‑`ended` ישן לא יעצור טראק חדש | ⬜ פתוח | `players.ts:245` — הסינון קודם לפיצול לפי סוג, ו‑`ended` מטופל ב‑248 |
| 2 | **חוסם — ספירת ערוצים.** מונו יצא מרמקול שמאל בלבד (נמדד ‑7.4 / ‑999 dBFS מול ‑7.4/‑7.4 במסלול הישן); קובץ עם יותר מ‑2 ערוצים זרק TypeError **בתוך** `process()`, מה שמכניס את המעבד לשגיאה קבועה — דק אילם עד סוף חייו | ✅ תוקן ובקומיט | `scratch-processor.ts:146,184,187` |
| 3 | **קיפול ערוצים מעבר לשניים ב‑`load`** — עדיין נזרקים במקום להתמזג ל‑L/R. **וההערה שמעליהם כבר טוענת שהם מקופלים** ("a surround file must not quietly lose its centre channel"), כך שמי שיקרא אותה יאמין שזה סגור ויפסיק לחפש. זה מופע חדש של "לא מדלגים בשקט" — בהערה עצמה | ⬜ פתוח — **לתקן ראשון** | `players.ts:256‑266` (`Math.min(srcChannels, 2)`) |
| 4 | **`node.onprocessorerror` לא מחובר** ל‑`scratchError` ולפיל ב‑TopBar, אז מוות של המעבד נשאר שקט | ⬜ פתוח | אין ולו הופעה אחת של `onprocessorerror` ב‑`src/`; התשתית כבר קיימת ב‑`engine.ts:98` ו‑`TopBar.tsx:87` |

נמדד עם ffprobe על הספרייה האמיתית: 357 מתוך 360 קבצים נבדקו, **כולם סטריאו** — אז 1–4
לטנטיים היום, אבל אקפלות וסטמים ווקאליים הם מונו באופן שגרתי.

**ארבע אזהרות שהועברו ולא אימתתי:** (א) `ctxTimeSec` שהמעבד שולח נזרק, ו‑`WorkletPlayer`
מטביע במקומו את זמן ההגעה ב‑main thread — הפלייהד מפגר ורועד עם עומס (~43 פעם בשנייה),
נסיגה מול המסלול הישן שהיה מדויק, ועוד סטייה קבועה של ~2.9ms. (ב) החלפת הנגן ב‑`Deck.load`
משחררת לפני שהיא בונה, אז חריגה משאירה את הדק על נגן משוחרר עם buffer ריק. (ג) סוף לופ
מעבר לסוף הטראק לא נחתך — אודיו, פער, חזרה; המסלול הישן קיבל חיתוך מהספֶּק בחינם.
(ד) הערות ב‑`controls.ts` וב‑`deck.ts` מנמקות את סדר הטעינה ב‑"transfer detaches the
AudioBuffer", אבל `players.ts` משתמש ב‑`copyFromChannel` דווקא כדי ש**לא** ינותק —
הנימוק הרשום שגוי, ומי שיבדוק אותו עלול לבטל את הסדר.

**חסר ל‑definition of done:** אף אחד לא הכשיל את ה‑worklet בכוונה ולא תיעד שה‑TopBar
מציג `no scratch · <reason>`. זה לא יכול היה לרוץ בהרצה מוצלחת.

**הכתיבה המלאה** יושבת ב‑scratchpad של שיחה `68c7df3f` (זמני — להעתיק אם צריך לשמור):
`v020a-review-findings.md`, `v020-evidence.md`, `worktree-mixed-v020b.patch`.
