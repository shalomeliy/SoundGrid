...
| | |
| --- | --- |
| **בעבודה** | `v0.5.5` שלב 1 סגור. שלב 2 (מודל מקומי, CPU-only מ-09/09 אחרי ש-GPU תקע שבב אמיתי) עבד פעם אחת ("נגן דק A", ~דקה), נתקע 120s בפעם השנייה. ביטלנו "חשיבה בקול" של Qwen3 (חשוד כגורם) — **עדיין איטי מאוד לפי שלום.** שלום הציע אזהרה על "מינימום כרטיס גרפי" — **תוקן במקום זאת: לא נכון לומר את זה, כי כרטיס גרפי מכובה בכוונה (הוא מה שתקע את השבב בפעם הראשונה).** הבנה נכונה יותר: ה-CPU לא רץ עם threads מרובים כי חסרים כותרות HTTP מסוימות (`crossOriginIsolated`). **תוקן:** `vite.config.ts` מקבל `Cross-Origin-Opener-Policy`+`Cross-Origin-Embedder-Policy:'credentialless'` (`server`+`preview`) — אומת כאן: `crossOriginIsolated`+`SharedArrayBuffer` נכונים, האפליקציה עולה תקין, זרימת ה-AI (אזהרה→טעינה→שגיאה) עדיין עובדת. **סיכון לא-מאומת:** `credentialless` יכול תיאורטית לא לעבוד חלק עם ה-CDN של huggingface.co — נבדק רק כאן, בלי רשת אמיתית. |
| **branch** | **מ‑09/09: עובדים ישירות על `main`, בלי branch נפרד, בכל שיחה** (הוראת שלום מפורשת). לבדוק: `git status` מראה `On branch main`. |
| **גרסה נוכחית** | `v0.5.4` (`package.json`) — שלב 2 עדיין לא סגור. |
| **`npm run check`** | ירוק — מלבד אותם 8 כשלים pre-existing ב‑`doc-commits.test.ts` (שיבוט git רדוד), לא קשור לדיף. |
| **הבא בתור** | שלום עושה `git pull`, מפעיל מחדש את `npm run dev` (השינוי ב-`vite.config.ts` לא נכנס בלי הפעלה מחדש), ומנסה שוב. **שתי תוצאות אפשריות שחשוב לדעת:** (1) מהיר יותר ועקבי — ממשיכים למדידת קורפוס וסוגרים את הגרסה; (2) ההורדה עצמה נשברת (שגיאת רשת שלא הייתה קודם) — סימן ש-`credentialless` לא מתיישב עם huggingface.co, ואז חוזרים ל-`vite.config.ts` ומורידים את שתי הכותרות. אם זה עדיין איטי בלי שגיאה חדשה — זו מגבלת CPU אמיתית, לא באג, והשאלה חוזרת לשלום: לחיות עם זה או לשקול BYO-key. |
