...
| | |
| --- | --- |
| **בעבודה** | `v0.8.8` **נסגרה 17/09** — שלום בדק במיקס אמיתי, אישר שעובד.
  פרטים: [`docs/handoff/v0.8.8.md`](docs/handoff/v0.8.8.md). `v0.8.5` M3
  ממוזג, שלום אישר שהמנגנון עובד — סגירה רשמית ממתינה לאישור על **איכות**
  התוצאות (הקריטריון המדיד ב‑`PLAN.md`). GridFix נבנה חלקית והוסר (16/09). |
| **⚠ לקח 17/09** | `v0.8.6` סומן ✅ אבל קומיט אחד לא הגיע ל‑main, נשאר
  ב-branch יתום. שוחזר (PR #27). **"✅" לא מוכיח מיזוג — `git log
  origin/main..<branch>` כן.** |
| **branch** | `main`, הכל ממוזג (`git log --oneline -1`). |
| **גרסה נוכחית** | `v0.8.8` (`package.json`) — נסגרה. |
| **`check`** | ירוק — 527/527. |
| **הבא בתור** | v0.8.9 (ייצוא בנק סאמפלר) — או קריטריון סגירה מפורש ל‑v0.8.5. |
